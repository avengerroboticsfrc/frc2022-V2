...
public class FiveBallAuto extends SequentialCommandGroup {
  public FiveBallAuto(DriveTrain drive, Limelight limelight, Shooter shooter, Intake intake, Index index,
    ...
    PathPlannerTrajectory path = PathPlanner.loadPath("FiveBallAuto", 3.25, 3);
    ...
    commands.put(6.041816317536586,
        new PickUpBallCommandGroup(intake, inIndex, index, intakePower, intakeToIndexPower, indexPower).withTimeout(2));
    addCommands(
        new InstantCommand(() -> drive.resetOdometry(path.getInitialPose())).withTimeout(0.5),
        new FridayRamseteCommand(path, drive, commands),
        new IntakeAndShootCommandGroup(
            shooter,
            index,
            limelight,
            intake,
            inIndex, inShooter, shooterPower, shooterPower, shooterPower, shooterPower).withTimeout(4),
