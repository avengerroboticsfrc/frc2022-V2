...
public class ThreeBallTimeBasedAuton extends SequentialCommandGroup {
  public ThreeBallTimeBasedAuton(DriveTrain drive, Intake intake, Index index,
      IntakeToIndex intakeToIndex, Shooter shooter, IndexToShooter indexToShooter,
      double intakePower, double indexPower, double intakeToIndexPower, double indexToShooterPower,
      Limelight limelight) {
    addCommands(new RunCommand(() -> drive.tankDrive(0.5, 0.5), drive).withTimeout(1.5),
        parallel(
            new RunCommand(() -> drive.tankDrive(0.5, 0.5), drive).withTimeout(1)
                .andThen(() -> drive.tankDrive(0, 0)),
            new PickUpBallCommandGroup(intake, intakeToIndex, index, 1, 0.5, 0.5)).withTimeout(2),
